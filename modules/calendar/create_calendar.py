...
def create_calendar(entry_frames: dict = {}, frames_dict: object = ctk.CTkFrame, root: object = ctk.CTk):
    ...
    all_buttons = []
    print(f'Это все ентри фреймы: {entry_frames}')
    for day in range(count_days):
        ...
        button = Button(
            ...
            size = 30,
            ch_corner_radius = 7,
            ch_command = lambda day = day + 1: (select_button(index_button = day - 1, all_buttons = all_buttons), 
            swith_frame(root = root, frame_name = 'CREATE_NOTE_HEADER'),
            write_json(filename = "utility.json", obj_dict = day)
                ))
